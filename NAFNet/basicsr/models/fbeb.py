...
class FrequencyBandEnhancementBlock(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,
        return_band_features: bool = False,
    ) -> torch.Tensor | tuple[torch.Tensor, tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
        identity = x
        x_norm = self.norm(x)

        fft_raw = torch.fft.fft2(x_norm.float(), dim=(-2, -1))
        fft_x = torch.fft.fftshift(fft_raw, dim=(-2, -1))
        ...
        phase = torch.angle(fft_raw)
        phase_only_fft = torch.polar(torch.ones_like(phase), phase)
        structure_prior = torch.fft.ifft2(phase_only_fft, dim=(-2, -1)).real.to(dtype=x.dtype)
        attn_weight = self.phase_attn(structure_prior)

        spatial = self.spa_conv1(x_norm)
        spatial = F.gelu(spatial)
        spatial = self.spa_conv2(spatial)
        spatial = spatial * attn_weight
        ...
        self._last_band_stats = {
            ...
            "phase_gate_mean": attn_weight.mean().detach(),
        }
